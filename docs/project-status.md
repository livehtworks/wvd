...
- 已将新帧确认接入 WvdRunState 和有界诊断；业务操作去重、错误任务步拒绝和根检查点有实际 Maa 调用。宝箱有限段包含角色/恐惧判断、逐点检查和转场撤销，只有返回地下城确认后计数；更多尝试与完整战斗分派仍待接入。首轮 6 个方法中 4 通过，修复选人候选抢占后 3 个定点方法通过，原状态 7 项回归通过，见 `../next/docs/m4-confirmation-validation.md`。继续世界地图往返、补给及后续业务。
- 已在 `1506f00` 后补入地图目标有限段与子流程组合：搜索滑动/排除区、到点与楼层判断、移动采样、Auto-Move 冻结出口、遇敌撤销旧点位。首轮 18 个流程方法中 13 通过；补全测试夹具后 5 个地图方法定点复测通过。证据见 `../next/docs/m4-navigation-validation.md`。完整任务通过数仍为 0/58，继续后续业务接入，不以本阶段提交为整包收口。
- 当前 M4 已增加有限 Pipeline 编译/发布、世界地图入城子流程、旅店住宿子流程、补给条件和关闭技能覆盖层后启用 Auto 的子流程。当前工作根为 `next/.local/m4-business-038717bea778422ba001a95ffec4e8c6/`；最终构建通过 30 个 M4、100 个 M2、23 个 M1、4 个 M3 定点测试和 4 个浏览器场景。450 个保护文件无变化。没有实机操作，前轮审核包保持不变。范围见 `../next/docs/m4-workflow-validation.md`。

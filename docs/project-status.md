...
- 无因果时间跳跃五方法/八场景通过（`e3d04e2`）；半自动大恶魔双段图的状态25方法与计划8方法通过，但流程两次修正后仍有帧龄/确认过期，保留BLOCKED，不再重跑。`74b99fb`同构建M2 104方法、地图后置/移动谓词两方法通过；不覆盖完整任务失败。悬赏揭榜/交付修正一七场景通过（`d88a348`），尚未接完整蝎女等专项；连续住宿9999次的250段实现待验。
...
| M4.7–4.8 | 陷阱七点链；巨人/暗灯/半自动大恶魔阻断；挖矿入口、连续周期和Pause恢复分项通过；悬赏菜单通过；连续住宿实现待验；RunStore逐代次摘要与善恶CAS | 三专项阻断、挖矿未知后置、连续住宿回归及其它九专项实际链、收入/交付/补饵、真静止后的任务转交、58项完整矩阵与最终回归 |

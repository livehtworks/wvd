...
- Pause 覆盖层先于底层就绪图标处理，六次无效点击后新帧仍 Pause 才请求恢复；首次/第六次成功和角色/详情负例通过，补验及普通阻塞/启动恢复 21 方法通过，见 `../next/docs/m4-pause-validation.md`，实现提交 `a8b1bd1`。复活消费者与失败遭遇独立编号已接入源码，构建/验证进行中，尚不计通过。

...
- 当前 M4 子流程包括有限图编译/发布/组合、地图搜索与移动确认、世界地图往返、组队后住宿、关闭技能覆盖层后启用 Auto、宝箱逐点防转场及新帧业务确认。世界往返/补给最新 5 个方法通过，详见 `../next/docs/m4-travel-validation.md`；地图、确认和宝箱证据分别见 `m4-navigation-validation.md`、`m4-confirmation-validation.md`（均在 next/docs）。任务步去重和根检查点已接入，完整任务分派与技能/恢复仍未接齐，完整通过仍为 0/58；每阶段提交后继续，不作为整包停止条件。

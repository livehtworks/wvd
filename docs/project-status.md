...
- 原生子流程组合已接入有限遭遇战：显式清理子图命中计数，不重置父预算/策略；子输入失败不被恢复出口覆盖。整组 M4 91 方法通过；最终构建补验 11 个编译/子任务/遭遇方法、M2 100、M3 定点 3、状态 10、计划 4 方法通过；见 `../next/docs/m4-native-child-validation.md`。继续角色面板恢复、持续 Auto 等待及完整路线，完整任务仍 0/58。

...
- 资源入口审计复现原生Pipeline模板/OCR漏检成员变化；跨包同名模板、延后调用与九项lease负例通过。已补原生开始事件检查、单次检查与首因保留，中途变化负例待构建复验，见 `../next/docs/m3-integrity-entry-validation.md`。不得把修正前产物结果套到新代码。
- 自动移动地图依赖修正后六方法/十一场景通过（`197f531`）；业务确认帧龄检查五场景及M2 102方法通过（`33561a3`）。陷阱专项修正夹具遗漏的六次重开地图滑动后，完整33次输入、冷启动33次、中途恢复49次已通过，连续两轮与最终汇总仍在运行。对应地图、确认时效与陷阱专题报告保留失败轮次。
- 默认对话十四选项和住宿付款待确认保护已实现并分别提交检查点，**尚未构建/执行新增测试**。见 `../next/docs/m4-default-dialogue-validation.md`、`../next/docs/m4-inn-payment-validation.md`。
...
| M4.7–4.8 | 陷阱专项七点链；RunStore逐代次输入/业务摘要；善恶确认CAS写回新版profile | 其它十四专项实际链、收入/交付/补饵/跨周期住宿、真静止后的任务转交、58项成功/失败/停止/恢复及最终完整回归 |

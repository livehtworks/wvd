...
- 当前 M4 子流程包括有限图编译/发布/组合、地图搜索与移动确认、世界地图往返、组队后住宿、关闭技能覆盖层后启用 Auto、宝箱逐点防转场及新帧业务确认。只读业务条件已接入 Maa 分派，同帧状态缓存失效和普通战斗插入后原步续接通过；8 个状态、1 个分派及 3 个 M3 定点方法证据见 `../next/docs/m4-state-routing-validation.md`。地图、补给和确认的阶段证据分别见 `m4-navigation-validation.md`、`m4-travel-validation.md`、`m4-confirmation-validation.md`（均在 next/docs）。完整任务分派与技能/恢复仍未接齐，完整通过仍为 0/58；每阶段提交后继续，不作为整包停止条件。

...
- 回城补给与本轮住宿回执已接入：8 方法/15 场景及状态 11 方法通过，见 `../next/docs/m4-departure-validation.md`。保留普通/强制补给和不同入口条件；已确认住宿不重复付费。完整 EOT/路线循环和其余全局事件仍待接齐，不是完整任务通过。
- 地下城路线组合已连接任务点、战斗、宝箱、恢复和退场确认；43 项路线静态编译与资源核对通过，8 个合成路线场景通过，见 `../next/docs/m4-dungeon-route-validation.md`。Session 总预算进入封存定义，未放宽帧 TTL；完整任务仍 0/58，继续出本/回城/住宿与完整任务闭环。
- Auto 持续等待已补齐：独立轮次预算、关闭后重新观察、等待中停止与输入失败不结算；20 个定点方法通过，见 `../next/docs/m4-auto-wait-validation.md`。继续角色面板恢复和完整任务链，不把阶段通过当整包完成。
- 角色面板恢复已接入有限 Pipeline 与运行状态：7 方法/11 场景通过，M2 100、状态 11 方法回归通过，见 `../next/docs/m4-healing-validation.md`。保留首轮接线和夹具失败证据，下一步接完整路线及遭遇后的恢复调度。
- 原生子流程组合已接入有限遭遇战：显式清理子图命中计数，不重置父预算/策略；子输入失败不被恢复出口覆盖。该阶段整组 M4 91 方法通过，随后编译/子任务/遭遇及必要回归通过；见 `../next/docs/m4-native-child-validation.md`。后续 Auto 和角色恢复证据分列，完整任务仍 0/58。
- 类型化恢复与启动后继续原任务已补离线验证：11 方法/16 场景通过，见 `../next/docs/m4-lifecycle-validation.md`。首次连接失败冷启动、完整任务续接和真实端口仍未齐，不把该组当 M4.6 或整包完成。
- 已补有限多角色遭遇战、连续遭遇幂等计数和战斗结束时撤销 Auto：25 方法/36 场景及 10 个状态方法通过，见 `../next/docs/m4-encounter-validation.md`。恢复阶段同构建追加核心 100、状态 10、M3 定点 3、遭遇 6 方法回归通过；完整任务保持 0/58。

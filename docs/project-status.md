...
- 指定领取/忍洞首轮状态与计划40方法通过；12方法流程中3失败，定位为夹具遗漏滑动时长，已修正待复验。吉尔七场景、忍洞对话/停止等分项已核对旧产物身份。领取访问与跳跃意图由独立状态对象持有，不计完整任务通过。
...
- 钢试炼源码扩展已接入独立编译入口与离线夹具，待构建；没有向基础目录添加ID。状态对象、选择意图、四点和专用住宿规则独立于Maa，见`../next/docs/m4-steel-trial-validation.md`。其余三扩展尚未实现，钢试炼自身矩阵仍未通过。

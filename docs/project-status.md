...
- 挖矿有限链五方法/八场景通过（`228f2f0`），含冷启动、奖励去重和十八次输入的返城组队住宿；另全矿物/mod/ROI和错点负例通过。EOT帧龄问题修正二（`70e7d11`）的完整入口及自动寻路四方法/八场景通过，入口共11输入；保持中途恢复/连续周期及完整矩阵待验。
...
- 无因果时间跳跃五方法/八场景通过（`e3d04e2`）；半自动大恶魔双段图的状态25方法与计划8方法通过，但流程两次修正后仍有帧龄/确认过期，保留BLOCKED，不再重跑。`74b99fb`同构建M2 104方法、地图后置/移动谓词两方法通过；不覆盖完整任务失败。悬赏揭榜/交付子图正在验证，尚未接完整蝎女等专项。
...
| M4.7–4.8 | 陷阱七点链；巨人/暗灯阻断；挖矿含入口部分流程通过；半自动大恶魔双段图待复验；RunStore逐代次输入/业务摘要；善恶确认CAS写回新版profile | 两专项阻断、挖矿中途恢复/连续周期、半自动大恶魔及其它十专项实际链、收入/交付/补饵/跨周期住宿、真静止后的任务转交、58项完整矩阵与最终回归 |

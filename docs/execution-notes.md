...
- c1666a0 工作包要求仅构建受影响目标和限定实机正常路径，不跑 ctest/pytest/旧矩阵。FlowProgram schema 4：程序共享不可变所有权，辅助调用者须在修改测试图后重新显式封存，不能恢复生产值复制接口。技能性能的主线程 exclusive 可相加，worker_exclusive 不可与墙钟相加；普通内存日志限频后不能与旧高频峰值直接比较。
...
- 网络重试覆盖层需要在整个任务（含输入结果等待）观察，不只在启动页处理。繁中正文和“重試”按钮共同确认，英文保留独立模板；复用原生事件暂停父等待，退出后检查原后置条件，不重放提交/付款/跳轮。处理器总预算 180 秒，按钮消失等待最多 120 秒，再次出现按新帧重试；未知错误/维护页不得借通用确认按钮强行继续。
- 蝎女过夜循环用 `next/tools/run_scorpion_loop.ps1`，它只走正式 API；`%LOCALAPPDATA%/WvdNext/overnight/STOP` 阻止下一轮，不强行中断当前轮。失败会退出并尝试补一张现场截图，不自动重试、重连或重启模拟器。滚动历史帧在 `%LOCALAPPDATA%/WvdNext/recent-frames`，单轮完整日志与异常 PNG 仍在 `runs` 下；不要为清理历史而删除用户日志或运行记录。

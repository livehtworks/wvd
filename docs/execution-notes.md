...
- 用户已明确永久禁止无效测试。任何测试开始前必须能回答：它直接证明哪个用户可操作功能、对应哪条验收断言、预计最长耗时、失败停止条件和产物用途；答不清楚则不得执行。
- 新架构或迁移工作的第一验收对象是最小真实用户链，不是测试数量、覆盖率、构建状态或隔离组件数量。本项目当前最小链为：连接既定 MuMu -> 检查/开启 VPN -> 启动游戏 -> 处理真实启动页 -> 进入稳定游戏场景 -> 执行一个安全任务步骤 -> 用户可停止。
- 最小真实链任一环失败，立即停止所有下游、大规模、长时间、资源、压力、矩阵和重复验证；先修复该阻断并用最小复现证明。不得一边保留基础功能失败，一边继续累计与交付无关的 PASS。
- Mock、假设备、合成图片、离线流程、接口 2xx、编译通过、打包成功和进程启动，只能证明其直接覆盖的局部契约；不得据此宣称真实功能完成，也不得替代本轮明确要求的真实链。
- 禁止重复运行同一失败测试以挑选成功结果，禁止通过增加轮数、延长预算、放宽阈值或扩大重试解释失败。只有代码、环境或验收输入发生可说明的实质变化后，才允许一次有界复测。
- 实机测试默认单次、有限、可停止。未经用户针对本轮明确授权，不得自动重启模拟器、消费游戏资源、执行不可逆游戏操作或扩大任务范围。未知画面、NoHit 和业务检查点缺失本身不得升级为模拟器重启证据。
- 每轮开始先报告本轮唯一功能目标和最短验证路径；结束时首先报告该功能是否真实可用。若不可用，明确写未交付并停止，不用测试数量、工作时长或底层成果冲淡结论。
- 长测试必须设置明确上限并在执行前说明。没有直接功能价值的压力测试、穷举测试、重复内存测试和大矩阵测试一律不执行，除非用户明确指定该测试本身为交付目标。
...
- 当前正式发布契约为 `input_contract=2`：`GuardedAction` 只提交一次输入，派生的 `AwaitTransition` 独立观察页面结果，业务完成再由 WVD 事件和根检查点确认。不要把输入返回、页面到达和业务完成重新合并，也不要在未知结果时自动重放输入。
- 游戏区域设置可能是繁体中文，不能把英文文字模板命中能力外推到繁中页面。带文字的可点击目标按语言分别提供单一位置模板；`any` 等复合条件只用于观察，不能直接作为点击位置。
- 语言适配必须沿完整业务调用链盘点，不能只修当前失败节点。跨语言不变的城市建筑动作使用不含文字的亮色图标探针；带文字的按钮、章节、任务目标和菜单项进入统一语义资源契约。业务函数不得重新散落 `Inn/guild/EdgeOfTown` 等夹带英文文字的公共动作模板。
- 旅店、公会、铁匠铺等建筑按钮在各城市共用同一图标，只能证明“城市主界面”，不能证明具体城市。王城身份使用画面中部偏右的固定塔楼天际线 ROI 只读确认，不与公共建筑图标做双锚点推断。
- `InputPolicy.max_frame_age=0` 表示不启用统一输入寿命；非零值只适用于输入前证据。历史文档中“保留 2 秒 TTL”描述的是当时版本和失败记录，不再是当前点击后或业务证据契约。
- `AdbBackend::context_matches` 只复核连接、前台和 `dumpsys input` 的默认显示 viewport，不得重新截图、重连或猜测方向。当前 MuMu Android 15 的真实输出尚未验收；解析失败必须拒绝输入并保留预览。
- 原生定向测试运行前需把 `next/.local/maafw.json` 中 SDK 的 `bin` 加入当前进程 `PATH`；缺少 DLL 的 `-1073741515` 只表示加载失败，不能记录成断言结果。
- `AdbBackend::wait` 和部分第三方 Maa 销毁仍可能长时间不返回。项目自有观察循环可取消，不等于第三方调用可取消；超时时必须保持对象和设备租约，报告清理中或 `STOP_TIMEOUT`，不得 detach、强杀全局 ADB或伪报静止。

...
以下两列必须分开阅读。本文下半部分保留的是 `661069f` 基线的审查记录，不能当作当前实现描述。

| 历史失败归因 | 当前风险治理 |
| --- | --- |
| 旧 Run 的一次 `34,620,024` 字节申请失败属实；缺模板、实际 ROI、调用栈和当时内存标量。不能认定泄漏或已查明故障矩阵。 | 全屏 900×1600 BGR 的单张 `CV_64F` 积分图：`901 × 1601 × 3 × 8 = 34,620,024` 字节，精确吻合失败申请。普通 `TM_CCOEFF_NORMED` 可达双积分图路径；这是最强来源候选，不是旧现场证明。两张为 `69,240,048` 字节，四路同时持有的条件数为 `276,960,192` 字节，均未计其他矩阵。 |
| 旧现场没有 PrivateUsage、系统 Commit、活动分片、已解码资产字节，不能反推当时资源峰值。 | `recognition::MatchBudget` 在叶级重计算前按已知 ROI/模板/方法估算工作集，默认 256 MiB **调度目标**；合法单项超目标时独占，提交空间不足则报资源错误。通用、WVD、portrait 和浮标路径共用 Application 预算，不全局关闭 OpenCV 并行。估计含已知积分图/结果图、DFT 调度余量、masked/灰度/排除/缩放；不是 OpenCV 分配上界。 |
| 原缓存只按 2048 个条目限量，worker 冷图可重复解码；这说明设计有风险，不证明旧异常由缓存引起。 | `DecodedAssetCache` 在 Service 会话内共享，规范键包括资源包 revision/lease/成员路径和像素变换；同键单次装载，128 MiB 可重建像素留存目标与 retained/in-use/evictable/live 分账。`Cache::assets` 仅存运动、滚动和冻结窗口等不可按图像 LRU 淘汰的时序状态。静态 539 PNG 的理论 BGR8 合计 `31,683,318` 字节（约 30.22 MiB），不是热集或实测峰值。 |
| 旧 `one()` 在 OOM catch 内重新构造 JSON/字符串，可能使原异常被二次失败覆盖。 | 匹配、解码和预处理在平台层预开句柄/固定槽记录短 ID、ROI、算法、预算、缓存与进程/系统内存标量；`bad_alloc`/`StsNoMem` catch 先用固定缓冲写最小记录，保留 OpenCV 数字 code，再以 Error 终止。正常会话末向 Run journal 写 `recognition.resources`。低内存时完整 API JSON 仍不作保证。 |
| 历史记录不能证明新代码在 MuMu 下的峰值、真实取消和长期资源稳定。 | Release 增量构建与三个定向原生测试已通过；本轮未连接或操作游戏、未制造 OOM、未运行长测。`RESOURCE_UNRESOLVED` 保留，待用户选定自然现场只读检查 `recognition-memory.log` 和 session 汇总后再判断。 |

`next/.local/png-memory-inventory-661069f/png-memory-inventory.json` 为本轮只读 PNG 头清单。候选的源码 HEAD、未提交 diff、EXE、前端和资源哈希以 `next/dist/wvd-next-native/DELIVERY_STATUS.json` 为准；未自动提交或推送。

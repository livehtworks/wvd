...
std::wstring strToWstr(std::string str) {
    if (str.empty())
        return L"";
    const int count = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, str.data(),
                                          static_cast<int>(str.size()), nullptr, 0);
    if (count <= 0) throw std::runtime_error("OCR_MODEL_PATH_UTF8_INVALID");
    std::wstring wstr(static_cast<std::size_t>(count), L'\0');
    if (MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, str.data(),
                            static_cast<int>(str.size()), wstr.data(), count) != count)
        throw std::runtime_error("OCR_MODEL_PATH_UTF8_INVALID");
    return wstr;
}

Ort::Env &sharedOcrEnv() {
    static Ort::Env env{ORT_LOGGING_LEVEL_ERROR, "WvdOcr"};
    return env;
}

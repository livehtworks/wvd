...
void CrnnNet::initModel(const std::string &pathStr, const std::string &keysPath) {
    ...
    session = new Ort::Session(sharedOcrEnv(), crnnPath.c_str(), sessionOptions);
#else
    session = new Ort::Session(sharedOcrEnv(), pathStr.c_str(), sessionOptions);
#endif
    inputNamesPtr = getInputNames(session);
    outputNamesPtr = getOutputNames(session);

    //load keys
    std::ifstream in(std::filesystem::path(strToWstr(keysPath)));
    std::string line;
    if (in) {
...
TextLine CrnnNet::getTextLine(const cv::Mat &src) {
    ...
    auto outputTensor = session->Run(runOptions, inputNames.data(), &inputTensor,

...
J freeze_handoff_source(const runtime::RunDefinition &definition, const WvdProfile &profile,
    ...
    J source{{"schema", 1}, {"request_id", definition.request_id},
        ...
        {"profile_digest", digest_handoff_json(profile.values)}, {"profile_sources", profile.sources},
        {"selected_section", profile.selected_section},
        {"legacy_document_digest", digest_handoff_json(profile.legacy_document)},
        {"legacy_passthrough_digest", digest_handoff_json(profile.legacy_passthrough)}};
    source["digest"] = digest_handoff_json(source);
    return source;
}
...
std::optional<contracts::RunSnapshot> TaskHandoff::poll(const std::filesystem::path &destination) {
    ...
    next.request_id = "handoff-" + digest_handoff_json({{"source", provenance_.at("digest")}, {"intent", intent.at("intent_id")}});

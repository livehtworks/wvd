...
更新：2026-09-15。修改基线 `4d7c4fa`；旧行为核对基线 `6585f407`。
状态实现已落地，M4 整体仍为 `M4_PARTIAL_IMPLEMENTATION`，58 项完整任务执行通过数仍为 0。

## 最新定向回归

`7fa2957`构建的状态组 **39/39** 通过，属于最终48方法定向批次，证据为
`next/.local/logs/m4-7fa2957-final-targeted.log`。新增冻结恢复profile与崩溃阈值同源、
非法阈值类型/uint64越界拒绝、任务文字和上轮耗时的真实状态消费者验证。
`farm_target_text`来自冻结配置；`last_lap_seconds`首次为null、按旧结算边界更新，
正常续段和恢复不重置，不把整个Run用时当成上轮耗时。M2同产物104方法通过。
先前37方法缺当时身份的记录不补造；旧表中的接线进度属于初始阶段，
当前承接状态以[业务汇总](m4-business-validation.md)与[语义审计](migration/m4-semantic-audit.md)为准。
...
后续业务接线已有分项实现与证据，不再把本表的初始待接线状态当成当前未实现清单。
本轮不要求全任务矩阵，已知复杂图帧龄阻断及实机边界见[阶段收口](m4-stage-closeout.md)。

...
首轮修正产物`8651c50`：完整性六方法全通过（16.479秒，
`m3-integrity-lifetime-correction1.log`），同构建M2全组104方法通过（96.852秒，
`m3-path-contract-m2.log`），含新增模板路径两负例和资源持有/释放断言。
另外接入子任务override八场景：克隆/原上下文都允许声明文件、拒绝未知文件、
目录引用和伪造scope。新增组在`e3d04e2`产物上一方法/八场景通过，3.416秒；
`m3-override-workflow.log`，证据根`m3-fixes-5n69wj20`。有效文件由实际Maa子流程命中；
负例在调用子任务前拒绝，八例底层输入均为零，不与已测六方法混算。

...
第二轮修正检查点 `197f531` 的组合条件/门禁/ROI 定点 3 方法通过（27.754 秒，`m4-auto-scene-parallel-contract.log`）。自动地图和自动移动回归 6 方法/11 场景通过（415.982 秒，`m4-auto-map-workflow-final.log`，私有根 `m4-workflow-9malvsue`）；已在重新构建前核对 11 份 EXE 身份、退出码、终态、输入次数、quiescent 和坐标无偏差。成功、停止、开图输入失败及不同自动模式均按预期结束。

该产物还发现独立缺口：`auto-map-chest` 的任务点确认用了约 2433.96ms 前的帧，业务确认没有和输入门禁一样检查帧龄。因此这里只认可输入流程复测通过，不宣布整个确认契约已通过。下一阶段为业务确认补相同 TTL/代次/epoch 校验，原通过证据不覆盖后续源码。

...
| M4.3 任务编译 | 58 项类型化数据、43 项真实参数入本图及地下城路线图；有限图发布、资源/权限绑定、普通插入；原生子调用及局部预算、深度/作用域负例已验证 | 58 项完整任务图及专项 case 尚未齐；数据和子图不能算整任务 |
| M4.4 导航补给 | 地图搜索/到点/楼层、移动冻结出口、世界往返、入城立即停点、组队后实际住宿、角色面板恢复、入本及自动寻路；路线组合接入遭遇后的重识别/恢复 | 剩余对话/退场/回城住宿和完整任务调度未接齐；路线证据见 m4-dungeon-route-validation.md |
| M4.5 战斗宝箱 | 单角色技能/等级/敌我目标、覆盖层与 Auto 确认、持续 Auto 有界等待、有限多角色遭遇、开箱转战斗撤销旧输入 | 完整死亡/复活/Pause、旧开箱全部重试与全任务连接尚未齐；真实质量未验证 |
| M4.6 恢复生命周期 | 应用优先、重连/实例升级的类型化离线端口、启动就绪后继续原任务；11 方法/16 场景通过，见 m4-lifecycle-validation.md | 首次连接失败冷启动、全任务恢复位置、专项转交及其余冻结原因未接齐；真实恢复仍禁止 |

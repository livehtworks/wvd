...
| M4.3 任务编译 | 58 项类型化数据、43 项真实参数入本图；有限图发布、资源/权限绑定、普通插入；原生子调用及局部预算、深度/作用域负例已验证 | 58 项完整任务图及专项 case 尚未齐；数据和子图不能算整任务 |
| M4.4 导航补给 | 地图搜索/到点/楼层、移动冻结出口、世界往返、入城立即停点、组队后实际住宿、角色面板恢复、入本及自动寻路子图 | 剩余对话/退场/完整路线分派与补给调度未接齐；角色恢复证据见 m4-healing-validation.md |

...
| M4.5 战斗宝箱 | 单角色技能/等级/敌我目标、覆盖层与 Auto 确认、持续 Auto 有界等待、有限多角色遭遇；普通/quick 有限开箱完整链通过，含 40 次输入；Pause、单人/多人死亡提示、复活后恢复、沙人/祝福提示与继续路线已验证 | 其它全局事件、未知页旧盲点退路与全任务连接尚未齐；真实质量未验证 |
| M4.6 恢复生命周期 | 应用优先、重连/实例升级的类型化离线端口、启动就绪后继续原任务；首次连接失败 6 方法/7 场景、恢复全组 17 方法及 M2/M3 定点/状态回归通过，见 m4-cold-start-validation.md | 全任务恢复位置、专项转交及其余冻结原因未接齐；真实恢复仍禁止 |
| M4.7 专项任务 | 15个quest原始数据保留；陷阱历史7场景通过；巨人/暗灯有限链阻断；挖矿矿点循环、冷启动、去重、住宿补镐子及完整入口通过；无因果时间跳跃八场景通过；半自动大恶魔双段图与状态契约已接线 | 巨人/暗灯不再重跑；挖矿中途恢复/连续周期；半自动大恶魔地图动作帧龄修正；其它10专项实际链未完成，完整任务验收仍未放行 |
| M4.8 持久化诊断 | RunStore 已记录各有限段定义、业务摘要、检查点、逐代次输入计数及最终完成段数；离线配置副本 CAS；善恶确认写回状态 18、流程 6 方法/13 场景、配置数据 11、计划 6 方法通过 | 完整业务计数、恢复对账与专项业务诊断未接齐；善恶验证见 m4-karma-validation.md |

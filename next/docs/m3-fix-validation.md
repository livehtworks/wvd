...
初始检查点没有自动提交；后续用户已授权阶段本地 commit，不含 push/PR、旧版打包或真实设备查询/连接/输入，最终边界仍在 M5 前。
...
本表保留初次产物结果，不代表当前 HEAD 全量复测。后续资源封存负例 2 方法/新增 9 场景通过，见 [补验报告](m3-integrity-negative-validation.md)；发现链与资源稳定性阻断不变。

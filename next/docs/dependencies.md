...
| OpenCV 直接编译接口（M3） | 4.12.0 / MaaDeps v2.12.6 | `opencv.lock.json` 固定官方归档、头/库/运行时 hash；与 Maa SDK 同 DLL，不装第二份 ABI |
...
M3 为纯 C++ 专用视觉直接使用固定 OpenCV 头和导入库，运行时仍使用 SDK 已有的
`opencv_world4_maa.dll`。来源归档固定为 MaaDeps 发布的 `MaaDeps-x64-windows-devel.tar.xz`，
准备工具和 CMake 双重核对；归档许可证留在隔离依赖目录。本轮审核包不分发这些依赖二进制。

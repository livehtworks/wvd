...
CompiledWorkflow accept_featured_request(FeaturedRequest request, bool royal_suite) {
    ...
    if (request != FeaturedRequest::BullCave && request != FeaturedRequest::GoldenChest && request != FeaturedRequest::Fordraig)
        throw std::runtime_error("FEATURED_REQUEST_INVALID");
    const bool bull = request == FeaturedRequest::BullCave;
    const bool fordraig = request == FeaturedRequest::Fordraig;
    const bool check_accepted = bull || fordraig;
    const std::string target_name = fordraig ? "fordraig/RequestAccept" : bull ? "LBC/request" : "SSC/Request";
    C graph(fordraig ? "quest.featured.fordraig" : bull ? "quest.featured.bull_cave" : "quest.featured.golden_chest", std::chrono::seconds{240});
    if (fordraig) graph.use_dialogue(recovery::DialoguePolicy::Fordraig);
    const auto inn = C::image("Inn"), guild = C::image("guild"), menu = C::image("guildRequest");
    const auto featured = C::image("guildFeatured"), target = C::image(target_name);
    ...
    const J accepted{{"mode", "featured_request_accepted"}, {"image", target_name}, {"accepted", true}};
    ...
    for (int i = 0; i < count; ++i) {
        ...
        graph.swipe(name, list, list, check_accepted ? J{150, 1000, 150, 200} : J{150, 1300, 150, 200},
            i + 1 == count ? J{"FindTarget"} : J{"Scroll" + std::to_string(i + 1)});
        graph.delay_after(name, i + 1 == count ? 2000 : 1000);
    }
    graph.route("FindTarget", check_accepted ? J{"AlreadyAccepted", "Prepare", "FineScroll"} : J{"Prepare", "FineScroll"});
    if (check_accepted) graph.observe("AlreadyAccepted", accepted, {"Exit"});
    const auto ready = check_accepted ? C::all({target, eligible}) : target;
    ...
    graph.click("Select", ready, target, left_list, {"Exit"}, fordraig ? J{350, 180} : bull ? J{266, 257} : J{300, 150});

...
RawFrame InputGate::capture() {
    ...
    if (raw.device_id != policy_.device_id || raw.viewport_id != policy_.viewport_id ||
        raw.size.width <= 0 || raw.size.height <= 0 || raw.size.width > 16384 ||
    ...
    frame_ = {raw.device_id,
              ...
              epoch_,
              raw.size,
              policy_.recognition_size,
              std::chrono::steady_clock::now(),
              "BGR8"};
    application_ = raw.foreground_application;
    scene_.clear();
    permit_.reset();
    events_.emit(generation_, "frame.captured", {{"frame_id", frame_.frame_id}, {"epoch", epoch_}});
    return raw;
}
...
bool InputGate::same_frame(const FrameIdentity &f) const {
    return f.device_id == policy_.device_id && f.game_id == policy_.game_id &&
           f.pack_revision == policy_.pack_revision && f.viewport_id == policy_.viewport_id &&
           f.generation == generation_ && f.frame_id == frame_.frame_id && f.frame_id > 0 &&
           f.action_epoch == epoch_ && f.raw_size == frame_.raw_size &&
           f.recognition_size == policy_.recognition_size && f.captured_at == frame_.captured_at &&
           f.color_format == "BGR8" &&
...
Command InputGate::mapped(const Command &command) const {
    ...
    auto x = [&](int value) {
        return static_cast<int>(
            std::lround(double(value) * frame_.raw_size.width / policy_.recognition_size.width));
    };
    auto y = [&](int value) {
        return static_cast<int>(
            std::lround(double(value) * frame_.raw_size.height / policy_.recognition_size.height));
    };
...
bool InputGate::execute(const Command &command) {
    ...
    Command raw;
    {
        ...
        auto reason = reject_reason(command);
        permit_.reset();
        if (!reason.empty()) {
            ++counts_.rejected;
